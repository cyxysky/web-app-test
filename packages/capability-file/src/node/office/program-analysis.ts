...
async function pythonDiagnostics(source: string): Promise<OfficeProgramDiagnostic[]> {
  ...
  try {
    ...
    const script = [
      ...
      ' def __init__(self, wrappers, spreadsheet_sheets): self.env={}; self.seen=set(); self.wrappers=wrappers; self.spreadsheet_sheets=spreadsheet_sheets',
      ...
      '  if not geometry and name.startswith("add_") and receiver not in self.spreadsheet_sheets:',
      '   box = call_value(node,99,"box")',
      '   if isinstance(box,(ast.Tuple,ast.List)) and len(box.elts) >= 4:',
      '    geometry = list(zip(("x","y","width","height"),box.elts[:4])); inch_box = True',
      ...
      '    if all(value is not None for value in values): geometry = list(zip(("x","y","width","height"),values)); inch_box = True',
      '  if not geometry and isinstance(node.func,ast.Name) and name in self.wrappers:',
      ...
      '  if inch_box and len(geometry) == 4:',
      '   numbers = [static_number(value,self.env) for _,value in geometry]',
      '   if all(value is not None for value in numbers):',
      '    x,y,width,height = numbers',
      '    if x + width > 13.334 or y + height > 7.501: add("PRESENTATION_GEOMETRY_OUT_OF_BOUNDS",node,f"Presentation {name} box ({x}, {y}, {width}, {height}) exceeds the 13.333 x 7.5 inch slide.","error")',
      '  self.generic_visit(node)',
      ...
      ' entrypoint = matches[0] if len(matches) == 1 and isinstance(matches[0],ast.FunctionDef) else None',
      ...
      '   if isinstance(value,ast.Call) and isinstance(value.func,ast.Attribute) and isinstance(value.func.value,ast.Name) and value.func.value.id == "job" and value.func.attr in {"presentation","writer","spreadsheet"}:',
      '    facade_vars.update(target.id for target in targets if isinstance(target,ast.Name))',
      '  if not facade_vars: add("PYTHON_DOCUMENT_FACADE_MISSING",entrypoint,"create_document(job) must create the requested Office document with job.presentation(...), job.writer(...), or job.spreadsheet(...).","error")',
      '  lifecycle_calls = [(candidate.func.value.id,candidate.func.attr,candidate) for candidate in ast.walk(entrypoint) if isinstance(candidate,ast.Call) and isinstance(candidate.func,ast.Attribute) and isinstance(candidate.func.value,ast.Name) and candidate.func.value.id in facade_vars]',
      '  for facade_var in sorted(facade_vars):',
      '   save_calls = [candidate for receiver,method,candidate in lifecycle_calls if receiver == facade_var and method == "save"]',
      '   close_calls = [candidate for receiver,method,candidate in lifecycle_calls if receiver == facade_var and method == "close"]',
      '   if len(save_calls) != 1: add("PYTHON_OUTPUT_SAVE_MISSING" if not save_calls else "PYTHON_OUTPUT_SAVE_DUPLICATE",entrypoint,f"create_document(job) must call {facade_var}.save() exactly once; found {len(save_calls)}. Without save(), the requested Office output file is never created.","error")',
      '   if len(close_calls) != 1: add("PYTHON_OUTPUT_CLOSE_MISSING" if not close_calls else "PYTHON_OUTPUT_CLOSE_DUPLICATE",entrypoint,f"create_document(job) must call {facade_var}.close() exactly once after save(); found {len(close_calls)}.","error")',
      '   if len(save_calls) == 1 and len(close_calls) == 1 and (save_calls[0].lineno,save_calls[0].col_offset) >= (close_calls[0].lineno,close_calls[0].col_offset): add("PYTHON_OUTPUT_LIFECYCLE_ORDER_INVALID",close_calls[0],f"{facade_var}.save() must appear before {facade_var}.close() so the output is written before the component is closed.","error")',
      ...
      '     if receiver_type == "PresentationSlide" and method == "connect":',
      '      for endpoint_name,endpoint in (("source_box",call_value(node,1,"source_box")),("target_box",call_value(node,2,"target_box"))):',
      '       if isinstance(endpoint,ast.Dict):',
      '        keys = {key.value for key in endpoint.keys if isinstance(key,ast.Constant) and isinstance(key.value,str)}',
      '        missing = [label for label,aliases in (("x",{"x"}),("y",{"y"}),("width",{"width","w"}),("height",{"height","h"})) if not keys.intersection(aliases)]',
      '        if missing: add("PRESENTATION_GEOMETRY_INVALID",node,f"PresentationSlide.connect {endpoint_name} is missing {missing[0]!r}; pass a prior child element ID or a complete x/y/width/height box.","error")',
      '     if receiver_type == "PresentationSlide" and method == "add_chart":',
      '      chart_type_node = call_value(node,1,"chart_type")',
      '      chart_type = str(chart_type_node.value).strip().lower() if isinstance(chart_type_node,ast.Constant) and isinstance(chart_type_node.value,str) else ""',
      '      chart_box = call_value(node,99,"box")',
      '      if isinstance(chart_box,(ast.Tuple,ast.List)) and len(chart_box.elts) >= 4:',
      ...
      '       if label_modes > 1: add("PRESENTATION_CHART_LABEL_DENSITY",node,"Pie/donut charts may enable only one of show_values, show_category_name, and show_percent; use legend + percent-only labels or category labels without a legend.","error")',
      '     if receiver_type == "PresentationSlide" and method == "add_shape":',
      '      shape_type_node = call_value(node,99,"shape_type")',
      '      element_id_node = call_value(node,0,"element_id")',
      '      shape_type = str(shape_type_node.value).strip().lower() if isinstance(shape_type_node,ast.Constant) and isinstance(shape_type_node.value,str) else ""',
      '      if isinstance(element_id_node,ast.Constant) and isinstance(element_id_node.value,str): element_hint = element_id_node.value.lower()',
      '      elif isinstance(element_id_node,ast.JoinedStr): element_hint = "".join(str(value.value).lower() for value in element_id_node.values if isinstance(value,ast.Constant) and isinstance(value.value,str))',
      '      else: element_hint = ""',
      '      if shape_type == "triangle" and any(token in element_hint for token in ("arrow", "conn-head", "connector-head")): add("PRESENTATION_CONNECTOR_ARROWHEAD_INVALID",node,"The separate triangle arrow tip is in the wrong position: touching the target with the triangle box does not place its top-center apex on the connector endpoint. Use slide.connect(..., end_arrow=True) so the native arrow tip terminates at the target edge.","error")',
      '     if receiver_type == "PresentationSlide" and method in {"add_text","add_rich_text","add_bullets","add_link"}:',
      '      text_keys = {"font_size","fontSize","min_font_size","minFontSize","bold","italic","underline","strike","strikeout","color","fontColor","fontColour","font_name","fontFamily","fontFace","align","textAlign","valign","verticalAlign","padding","line_spacing","lineSpacing","background","backgroundColor","background_transparency","backgroundTransparency","fill","fillColor","fill_transparency","fillTransparency","border","borderColor","line","lineColor","line_width","lineWidth","borderWidth","link","rotation","rotate","layout_role","layoutRole","allow_overlap","allowOverlap","fontWeight"}',
      ...
      '      if unknown: add("UNO_API_ARGUMENT_INVALID",node,f"Unsupported presentation text style key(s): {\', \'.join(unknown)}. Use padding, line_spacing, min_font_size, box geometry, or auto_height; query unoApi module presentation.text for the exact vocabulary.","error")',
      ...
      ' spreadsheet_layouts = set()',
      ' spreadsheet_sheets = set()',
      ' for candidate in ast.walk(tree):',
      '  if not isinstance(candidate,(ast.Assign,ast.AnnAssign)): continue',
      '  value = candidate.value',
      '  targets = candidate.targets if isinstance(candidate,ast.Assign) else [candidate.target]',
      '  names = [target.id for target in targets if isinstance(target,ast.Name)]',
      '  if isinstance(value,ast.Call) and isinstance(value.func,ast.Attribute) and isinstance(value.func.value,ast.Name) and value.func.value.id == "job" and value.func.attr == "spreadsheet": spreadsheet_layouts.update(names)',
      ' for candidate in ast.walk(tree):',
      '  if not isinstance(candidate,(ast.Assign,ast.AnnAssign)): continue',
      '  value = candidate.value',
      '  targets = candidate.targets if isinstance(candidate,ast.Assign) else [candidate.target]',
      '  names = [target.id for target in targets if isinstance(target,ast.Name)]',
      '  if isinstance(value,ast.Call) and isinstance(value.func,ast.Attribute) and isinstance(value.func.value,ast.Name) and value.func.value.id in spreadsheet_layouts and value.func.attr in {"sheet","select_sheet"}: spreadsheet_sheets.update(names)',
      ' GeometryVisitor(wrapper_geometry, spreadsheet_sheets).visit(tree)',
