...
export function configureNodeFileTextExtractionObserver(
  nextObserver: NodeFileTextExtractionObserver = {},
) {
...
const state: CpuPoolState = ((globalThis as typeof globalThis & {
  __capabilityFileTextExtractionPoolState?: CpuPoolState;
}).__capabilityFileTextExtractionPoolState ??= { nextId: 1, queue: [], slots: [] });
...
function updateGauges() {
  observer.setMetricGauge?.('cpu_worker_queue_depth', state.queue.length);
  observer.setMetricGauge?.('cpu_worker_active', state.slots.filter((slot) => slot.active).length);
}
...
function dispatch() {
  for (const slot of state.slots) {
    ...
    job.timeout = setTimeout(() => {
      ...
      observer.incrementMetric?.('cpu_worker_task_timeout_total', { task: 'attachment_extract' });
...
function createSlot() {
  ...
  slot.worker.on('message', (message: { error?: string; id: number; ok: boolean; value?: string }) => {
    ...
    slot.active = undefined;
    observer.recordMetricTiming?.('cpu_worker_task_ms', performance.now() - job.startedAt, {
...
export function extractFileTextInWorker(input: {
  extension: string;
  kind: FileTextExtractionKind;
  path: string;
}) {
  ...
  if (state.queue.length >= maximumQueued) {
    observer.incrementMetric?.('cpu_worker_task_rejected_total', { task: 'attachment_extract' });
...
export function nodeFileTextExtractionPoolSnapshot() {
  return {
...
export async function disposeNodeFileTextExtractionPool() {
  if (state.idleTimer) clearTimeout(state.idleTimer);
  state.idleTimer = undefined;
  const error = new Error('File text extraction pool disposed.');
  for (const job of state.queue.splice(0)) job.reject(error);
  const slots = state.slots.splice(0);
  await Promise.allSettled(slots.map(async (slot) => {
    if (slot.active?.timeout) clearTimeout(slot.active.timeout);
