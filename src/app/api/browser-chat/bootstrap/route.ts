...
export async function GET(request: NextRequest) {
  ...
  const sessionPage = await listBrowserChatSessionSummaries(userId, { limit: sessionLimit + 1 });
  const sessions = sessionPage.slice(0, sessionLimit);
  const lastSession = sessions.at(-1);
  const skillPage = await store.listSkills(undefined, userId, skillLimit + 1);
  ...
  return apiJson(request, {
    ...
    model: await readModelSettingsState(),
    runtime: (await readRuntimeSettingsItems()).filter((item) => browserChatRuntimeKeys.has(item.key)),
  });
}
