  ...
  async function restartTutorial() {
    ...
    try {
      await fetch(endpoint, {
        ...
        method: 'PATCH',
      }).then((response) => readApiJson(response, '重置教程失败'));
      setOpen(false);
            ...
            <div className="webpilot-help-content">
              <section>
                ...
                <ol className="webpilot-help-steps">
                  <li><Globe2 size={17} /><div><strong>描述浏览器目标</strong><p>说明要访问哪里、查什么或填写什么；提交和删除操作会按安全模式确认。</p></div></li>
                  <li><FileText size={17} /><div><strong>按需引用文件</strong><p>上传附件后说明目标。模型先看到元数据，只在任务需要时读取正文和页面图像。</p></div></li>
                  <li><Workflow size={17} /><div><strong>复用成功过程</strong><p>一次任务成功后，可从回答下方生成自动化用例或 Skill。</p></div></li>
                </ol>
              </section>
                ...
                {payload ? (
                  <div className="webpilot-help-readiness">
                    {Object.entries(payload.readiness).map(([key, item]) => (
                      <div key={key}><span className={item.ready ? 'ready' : 'warning'}>{item.ready ? <Check size={13} /> : '!'}</span><p>{item.detail}</p></div>
                    ))}
                  </div>
                ) : <div className="webpilot-help-loading"><Loader2 className="spin" size={16} />正在检查</div>}
              </section>

              <section>
                <h3>常见问题</h3>
                <details><summary>为什么有些操作需要确认？</summary><p>严格安全模式会在提交、删除、上传和其他会产生外部影响的操作前请求确认。</p></details>
                <details><summary>文件为什么没有自动全文读取？</summary><p>附件默认只提供名称、类型和大小，避免把无关大文件全部放进模型上下文。需要分析时模型会调用 readFile 分段读取。</p></details>
                <details><summary>浏览器登录状态在哪里？</summary><p>登录状态保存在当前用户的受控浏览器配置中；每个对话只接管属于自己标签组的页面。</p></details>
              </section>
            </div>

            <footer>
              <button className="ui-button secondary" disabled={resetting} onClick={() => void restartTutorial()} type="button">
                {resetting ? <Loader2 className="spin" size={15} /> : <RotateCcw size={15} />}重新开始教程
              </button>
              <Link className="ui-button secondary" href="/settings" onClick={() => setOpen(false)}><Settings size={15} />打开设置</Link>
            </footer>
          </aside>
        </div>,
        document.body,
