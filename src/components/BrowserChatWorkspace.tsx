...
export function buildAiCycleToolDetailMap(cycles: BrowserChatAiOutputCycle[], steps: StepExecutionResult[]) {
  ...
  cycles.forEach((cycle) => {
    ...
    cycle.output.tools.forEach((aiTool, aiToolIndex) => {
      ...
      const reasonMatch = sameNameCandidates.find((candidate) => (
        normalizedReason
        && candidate.tool.reason?.replace(/\s+/g, ' ').trim() === normalizedReason
      ));
      // Tool inputs can be normalized before execution (for example file reads
      // add a default limit). Only fall back to a same-name result when a
      // matching reason identifies it or it is the sole possible candidate.
      const detail = reasonMatch || (sameNameCandidates.length === 1 ? sameNameCandidates[0] : undefined);
      if (detail) {
        details.set(aiCycleToolKey(cycle.id, aiToolIndex), detail);
...
const BrowserChatAiCycleLine = memo(function BrowserChatAiCycleLine({
  ...
  return (
    <div className="browser-chat-ai-cycle">
      {/* Legacy grouped rendering retained here only as a merge reference; the ordered renderer below is authoritative. */}
      {/*
      ...
      {orderedParts.map((entry, orderedIndex) => {
        ...
        return (
          ...
          >
            ...
            <div className="browser-chat-ai-cycle-tools">
              <div className={`browser-chat-tool-call${tool.name === 'subagent' && toolInputValue(asRecord(tool.input), ['action']) === 'spawn' ? ' has-subagents' : ''}`}>
                {tool.name === 'subagent' && toolInputValue(asRecord(tool.input), ['action']) === 'spawn' ? (
                  <BrowserChatSubagentToolDisclosure
                    cardContent={card}
                    className={stateClass}
