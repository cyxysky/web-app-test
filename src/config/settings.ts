...
export const runtimeEnvDefinitions: RuntimeEnvDefinition[] = [
  ...
  { key: 'OFFICE_GENERATION_MODE', label: 'Office 文件生成模式', description: 'UNO 支持创建及原文件修改；JavaScript 使用 PptxGenJS、docx、ExcelJS 创建 PPTX/DOCX/XLSX，也可先生成对应 Office 中间文件再由本机 LibreOffice 转为 PDF；自动模式对这些新文件使用 JavaScript，修改现有文件仍使用 UNO。', tab: 'runtime', defaultValue: 'uno', control: 'select', options: [{ label: 'LibreOffice UNO（默认）', value: 'uno' }, { label: 'JavaScript Office 库', value: 'javascript' }, { label: '自动选择', value: 'auto' }] },
