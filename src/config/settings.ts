...
export const runtimeEnvDefinitions: RuntimeEnvDefinition[] = [
  ...
  { key: 'BROWSER_CODE_RUNTIME_MODE', label: '浏览器代码运行模式', description: '自由代码模式向模型暴露完整 Playwright page/context/browser；受限 API 模式只暴露文档化的 browserApi，并返回普通 JSON。切换后会自动重建代码内核。', tab: 'browser', defaultValue: 'free', control: 'select', options: [{ label: '自由代码模式', value: 'free' }, { label: '受限 API 模式', value: 'restricted' }] },
