...
class SpreadsheetLayout:
    ...
    def format_range(self, element_id, sheet, cell_range, font_size=None, bold=None,
        ...
        self.job.register_element(element_id, 'cell-format', target, {
    ...
    def add_chart(self, element_id, sheet, cell_range, box, chart_type='column', title=None, legend=True):
        area = PresentationLayout._rect(box, default_unit='in')
        target = sheet.getCellRangeByName(str(cell_range))
        ...
        name = 'wp_chart_' + re.sub(r'[^A-Za-z0-9_]', '_', str(element_id))[:80]
        charts = sheet.Charts
        ...
        if title:
            chart.HasMainTitle = True
            chart.Title.String = str(title)
        self.job.register_element(element_id, 'chart', embedded, {
            'sheet': str(sheet.Name), 'range': str(cell_range), 'chartType': key,
        })
        self.job.record_feature('nativeChart')
        return self
