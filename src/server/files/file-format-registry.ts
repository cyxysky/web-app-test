...
export type WebPilotFileFormat = {
  canGenerate: boolean;
  canPreview: boolean;
  canRead: boolean;
  canUpload: boolean;
  extension: `.${string}`;
  kind: WebPilotFileKind;
  mimeType: string;
};

type FileFormatDefinition = Omit<WebPilotFileFormat, 'canGenerate' | 'canPreview' | 'canRead' | 'canUpload'> & Partial<Pick<
  WebPilotFileFormat,
  'canGenerate' | 'canPreview' | 'canRead' | 'canUpload'
>>;

const text = (extensions: string[], options: Partial<FileFormatDefinition> = {}) => extensions.map((extension) => ({
  canGenerate: true,
  canPreview: false,
  canRead: true,
  canUpload: true,
...
export const webPilotFileFormats: readonly WebPilotFileFormat[] = Object.freeze(formats.map((format) => Object.freeze({
...
const formatsByExtension = new Map<string, WebPilotFileFormat>(webPilotFileFormats.map((format) => [format.extension, format]));
const formatsByMime = new Map<string, WebPilotFileFormat>();
for (const format of webPilotFileFormats) {
...
export function normalizedFileExtension(value: string) {
  const name = String(value || '').split(/[?#]/, 1)[0].replace(/\\/g, '/').split('/').at(-1) || '';
...
export function generatedFileExtensions(kind?: WebPilotFileKind): Set<string> {
  return new Set(webPilotFileFormats
    .filter((format) => format.canGenerate && (!kind || format.kind === kind))
    .map((format) => format.extension));
}

export function readableFileExtensions(kind?: WebPilotFileKind): Set<string> {
  return new Set(webPilotFileFormats
...
export function officePreviewExtensions(): Set<string> {
  return new Set(webPilotFileFormats
    .filter((format) => format.canPreview && ['presentation', 'spreadsheet', 'word'].includes(format.kind))
    .map((format) => format.extension));
}
