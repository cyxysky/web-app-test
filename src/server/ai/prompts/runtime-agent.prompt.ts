...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    allowedTypes.includes('file') ? '- For file, set params.action="read" with exactly one attachmentId/artifactId, action="download" with an existing URL/path and a required fileType extension without a dot, or action="generate" with a real output extension and content/sheets/slides. Inspect rendered output pages when returned and include only verified Artifact download links.' : '',
    allowedTypes.includes('skill') ? '- For skill, set params.action="read" and provide the exact params.skillId from an available <skill> summary before performing governed browser actions.' : '',
    allowedTypes.includes('subagent') ? '- For subagent, set params.action="spawn" with independent tasks, or for one child use flat params.title, params.url, and params.instruction; use params.action="read" with exactly one returned UUID. Never loop on the same rejected parameter shape.' : '',
