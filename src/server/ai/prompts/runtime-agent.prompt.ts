...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    snapshotMode ? '- Use takeSnapshot with params={mode:"actionable",maxChars:10000} for a fresh Chromium semantic DOM snapshot. Continue only with its opaque params.cursor; do not invent cursor values.' : '',
