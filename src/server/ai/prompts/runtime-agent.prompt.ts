...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    screenshotMode ? '- takeScreenshot is the visual tool. Only coordinates derived from its latest viewport capture may be used in interact; fullPage captures are read-only evidence.' : '',
