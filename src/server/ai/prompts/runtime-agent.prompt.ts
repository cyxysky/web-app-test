...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    browserCodeMode ? '- browserCode always returns the final page identity and console deltas. After an action, navigation, tab change, or failure it also returns a bounded postActionObservation with current dialogs/notices/focus and incremental DOM changes. Pure read cells skip that observation. Use targeted Playwright reads or page.domSnapshot() explicitly when more structure is needed.' : '',
    browserCodeMode ? '- Before every state-changing action, understand the exact user goal and confirm from fresh live evidence the current page, active dialog/layer, intended outcome, unique visible target, and loading or blocking state. Inspect within the same browserCode program or use the immediately preceding fresh result; if anything is uncertain, inspect instead of acting.' : '',
    browserCodeMode ? '- Default to tab.playwright.locator(...).click() or an equivalent stable locator on page. force: true is forbidden. If a locator click times out, do not fall back to CUA, page.mouse, DOM click, dispatchEvent, or script click; inspect postActionObservation and run a targeted fresh Playwright read or page.domSnapshot() for loading layers, popups, overlays, stale locators, detached elements, or asynchronous redraws.' : '',
