...
export function buildCodexObjectPrompt(
  prompt: string,
  allowedTypes: string[],
  runtimeMode: BrowserCodeRuntimeMode = resolveBrowserCodeRuntimeMode(),
) {
  ...
  const restrictedBrowserCodeMode = browserCodeEnabled && runtimeMode === 'restricted';
  const browserCodeMode = browserCodeEnabled && runtimeMode === 'free';
  return [
    prompt,
    '',
    'Codex local mode:',
    ...
    restrictedBrowserCodeMode ? '- browserCode is the live browser entrypoint in restricted API mode. Put one ordinary top-level-await JavaScript cell in params.code and use only browserApi, nodeRepl, and console. Playwright page/context/browser/tab/Locator objects and undocumented globals are absent.' : '',
    restrictedBrowserCodeMode ? '- The kernel persists across calls. Prefer top-level var or fresh names and emit compact JSON-safe results with nodeRepl.write(...). Keep cells bounded and use concrete state waits instead of routine fixed delays.' : '',
    restrictedBrowserCodeMode ? '- Follow the loaded system-browser-api-runtime signatures exactly. browserApi covers tabs, navigation, frames, snapshot/read/inspect, element actions, popup/navigation/dialog coordination, keyboard, pointer, upload, trusted credential fill, selection, screenshots, waits, form audit, surfaces, and verification.' : '',
    restrictedBrowserCodeMode ? '- Use readBrowserState as the initial observation. Every LocatorTarget field must be copied from exact current evidence. If evidence is absent or stale, call browserApi.snapshot/read/inspect before acting; never try a plausible selector as a probe.' : '',
    restrictedBrowserCodeMode ? '- Multiple bounded operations may run in one cell, but add targeted reads between dependent actions when earlier state changes later target assumptions. Resolve each opened nonmodal surface before targeting outside it.' : '',
    restrictedBrowserCodeMode ? '- User attachments are registered by attachmentId. Upload only with browserApi.act({target,action:"upload",attachmentId}); fill credential references only with action:"credentialFill". Never reconstruct files or read/output secret values or references.' : '',
    restrictedBrowserCodeMode ? '- Emit pixels with await browserApi.screenshot({fullPage:false}) and end that cell. A vision model may use one inspected fresh viewport image for multiple later coordinate clicks while geometry remains valid. A non-visual model must get one exact target rect through browserApi.read/inspect and click only inside it with browserApi.pointer().' : '',
    restrictedBrowserCodeMode ? '- After each cell inspect its result, dependencyFailures, and domChanges. Before claiming completion, perform a read-only business verification and call browserApi.surface(); successful action delivery alone is not business success.' : '',
    ...
    browserCodeMode ? '- The Playwright browser is server-side. page.evaluate Blob/object URLs, window.open, HTML download attributes, and page download clicks cannot deliver a file to the user browser. Use file action=download for an existing HTTP(S) resource. Create new files through action=plan → repeated small action=generate block batches → action=edit by block ID → action=render; never add a dummy block to trigger rendering. Choose a stable semantic documentId on the first plan of each distinct output and reuse it exactly for every repeated plan, retry, generation, edit, render, and visual correction; never replace the ID after an error. The document AST is strict: geometry and typography belong under block.style, visible text in block.text, SVG markup in block.svg, and image sources in block.source; invalid aliases fail rather than being guessed or moved. Freely compose text, images, SVG, charts, tables, cards, columns, metrics, timelines, shapes, pages, and sheets; there are no fixed templates or presets. PDF is a first-class LibreOffice-backed output. Only a rendered current-session Artifact download URL proves user delivery.' : '',
    ...
    allowedTypes.includes('file') ? '- For file, list/read/download are directly available. Before plan/generate/edit/render/convert/jsApi/unoApi, read system-file-artifact-runtime and then retry the original file call with the same documentId.' : '',
    ...
    allowedTypes.includes('subagent') ? '- Before subagent action="spawn", read system-subagent-runtime; action="read" is ungated and accepts exactly one returned UUID in the required order.' : '',
