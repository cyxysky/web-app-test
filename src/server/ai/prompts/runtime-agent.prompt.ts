...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    domMode ? '- DOM mode: use getPageState to collect the browser DOM tree into Node and refresh the current observation. Then use readObservation(type="text"|"interactive", offset, maxChars) to inspect Node-processed plain page text or actionable node_id entries. Use clickDomNode/fillDomNodes/hoverDomNode with current node_id values, or findByText then clickLocator for recovery.' : '',
