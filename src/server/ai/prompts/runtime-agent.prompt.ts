...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    allowedTypes.includes('readSkill') ? '- For readSkill, provide the exact params.skillId from one available <skill> summary. Read a relevant Skill before performing the browser actions it governs, and do not read the same Skill twice in one user turn.' : '',
