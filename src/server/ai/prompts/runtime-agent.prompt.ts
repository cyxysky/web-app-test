...
export function buildCodexObjectPrompt(
  ...
  return [
    ...
    browserStateGatePending ? '- If the request can be answered without the live browser, return type="answer" directly. If live browser evidence or interaction is needed, return the relevant browser operation directly. The execution layer runs pending prerequisites first, returns their complete results in prerequisiteResults, and still executes the requested operation in that same call. Return type="readBrowserState" only when the state itself is the desired result.' : '',
    ...
    browserCodeMode ? '- When the initial live-state preflight is pending, the execution layer runs readBrowserState internally, includes the conversation tabs, active tab/group, URL/title, and current snapshot in prerequisiteResults, and then executes browserCode in the same call. Return readBrowserState explicitly only when the snapshot itself is the desired result. Add a targeted read-only browserCode cell only when the exact locator/frame/surface evidence needed for the next action is absent or stale.' : '',
