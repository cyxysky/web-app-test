...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    allowedTypes.includes('file') ? '- For file, set params.action="read" with exactly one attachmentId/artifactId, action="download" with an existing URL/path, or action="generate" with a real output extension and content/sheets/slides. Inspect rendered output pages when returned and include only verified Artifact download links.' : '',
