...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    browserCodeMode ? '- page.domSnapshot() is an explicit read API returning the Playwright AX tree rooted at the page body. Operation, navigation, and tab-change results automatically return actual.finalPage, incremental actual.domChanges, and console deltas, but never an automatic axTree. The model decides when to call page.domSnapshot() or write targeted Playwright/DOM reads and returns only the evidence it needs.' : '',
    browserCodeMode ? '- Use exact attributes and hierarchy from the latest explicit AX/DOM/Playwright read or incremental domChanges to plan a bounded operation sequence. Multiple operations may run in one cell. If an earlier action can change later locator assumptions, use targeted Playwright reads before continuing. Current evidence wins over older tool output.' : '',
    browserCodeMode ? '- Never infer an element control type, editability, interaction sequence, or completion state from labels, appearance, or prior experience; use exact current tags, attributes, hierarchy, and newly mounted structure. For credentialVault.fill and every Playwright Locator/Page element action, build a targeted locator from current evidence. Page and Locator factory methods automatically remove matches hidden by themselves or an ancestor and matches without a non-empty rendered rectangle, so count(), first(), last(), and nth() operate on the rendered set. Runtime then independently runs full actionability checks on every remaining candidate and executes only the unique passing candidate. Pointer checks use the target final computed style, real hit testing, and an action-specific Playwright trial; an ancestor pointer-events:none alone does not reject the target. Other checks reject detached, disabled, inert, covered, opacity-hidden, aria-hidden, readonly/non-editable, or otherwise invalid targets. Hidden file inputs used by setInputFiles are the sole rendered-existence exception at the action boundary. first(), last(), and nth() are allowed when the model intentionally selects a positional candidate; the selected locator still receives normal validation.' : '',
    browserCodeMode ? '- After every operation cell, inspect its direct result and incremental domChanges. Autonomously decide whether to run an explicit page.domSnapshot(), URL, DOM, value, text, or network read before the next operation or final answer. Playwright delivery alone is not business success, but no specific verification helper is mandatory.' : '',
    browserCodeMode ? '- force: true is forbidden. After timeout, strict-mode, or actionability failure, inspect incremental domChanges or explicitly obtain fresh AX/DOM/Playwright evidence, then refine the locator; do not fall back to CUA, page.mouse, DOM click, dispatchEvent, or script click. Coordinates remain limited to the separately authorized visual-control flow.' : '',
