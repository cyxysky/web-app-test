...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    snapshotMode ? '- Use takeSnapshot with params={mode:"full"} by default for the complete semantic DOM baseline, params={mode:"text"} for deduplicated rendered copy, and params={mode:"changes"} only for the inter-action DOM/request journal. Text pages are fixed at 20,000 characters and full pages at 40,000 characters. changes has no interactive UIDs and does not replace the baseline. Query its request IDs with getHttpRequests. When nextCursor is returned, continue with the same mode and exact cursor; each response states its page number and total pages.' : '',
