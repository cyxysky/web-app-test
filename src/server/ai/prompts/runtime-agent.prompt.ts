...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    domMode ? '- DOM mode uses inspect for semantic page evidence, interact for one current snapshot-bound semantic/ref target or latest viewport coordinates, browser for navigation/tabs, and takeScreenshot only when pixel evidence is needed.' : '',
    ...
    domMode ? '- Begin browser work with type="inspect", params.action="capture", params.mode="full". For every DOM interaction, pass that inspect result snapshotId. Prefer a semantic target made from an exact name or stable attributes, with a unique scope parent when duplicates exist. The runtime acts only on one current actionable match. Use a short-lived ref target only when no stable semantic target exists; stale refs are rejected and never fuzzy-rebound. Use a fresh viewport screenshot and coordinates only for a purely visual control.' : '',
    domMode ? '- Ordinary target clicks never bypass actionability or covering layers. Only when a fresh inspect proves the exact intended action is to close the currently visible overlay may you send action="click" with params.force=true. Force skips only actionability and cover checks; snapshot ownership, live-node identity, and uniqueness remain mandatory. Never use force to make an uncertain target succeed; inspect again immediately afterward.' : '',
    domMode ? '- For a native select use type="interact", params.action="selectOption", params.snapshotId, params.target, and an exact params.value or params.label. For credentials use params.credentialRef with a current snapshot-bound field target; never put the secret in params.text.' : '',
