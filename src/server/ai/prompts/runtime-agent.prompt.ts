...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    browserCodeMode ? '- For exact middle insertion in an input, textarea, or contenteditable, including one located through frameLocator, call await page.insertTextAt(locator, { text, afterText | beforeText | offset, occurrence? }). It sets the caret and inserts through the real keyboard; do not use coordinates or direct DOM text mutation, and verify the resulting value/text.' : '',
    ...
    browserCodeMode ? '- browser/tab are code APIs in the same kernel: browser.tabs.list()/new()/finalize(), browser.user.openTabs()/claimTab(), tab.playwright, and tab.cua. openTabs returns only current-conversation-group tabs with active, groupId, groupTitle, URL, and title metadata; ungrouped and other-group tabs are excluded.' : '',
    ...
    domMode ? '- To insert text at an exact middle position, use type="interact", params.action="insertTextAt", the current editable or rich-text iframe dom-* ref, params.text, and exactly one of params.afterText, params.beforeText, or params.offset. Use params.occurrence for a repeated text anchor, then inspect again to verify the resulting text.' : '',
    ...
    allowedTypes.includes('generateFile') ? '- For generateFile, provide params.fileName with the real output extension. Use params.content for text/PDF/Word, params.sheets with two-dimensional rows for Excel, and params.slides for PowerPoint. The final visible answer must include the returned download link as a clickable Markdown link.' : '',
