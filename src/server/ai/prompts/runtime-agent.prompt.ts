...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    browserCodeMode ? '- The Playwright browser is server-side. page.evaluate Blob/object URLs, window.open, HTML download attributes, and page download clicks cannot deliver a file to the user browser. Use file action=download for an existing HTTP(S) resource. Create new files through action=plan → repeated small action=generate block batches → action=edit by block ID. Freely compose text, images, SVG, charts, tables, cards, columns, metrics, timelines, shapes, pages, and sheets; there are no fixed templates or presets. PDF is a first-class LibreOffice-backed output. Only a rendered current-session Artifact download URL proves user delivery.' : '',
