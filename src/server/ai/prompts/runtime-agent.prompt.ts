...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    snapshotMode ? '- Use takeSnapshot with params={mode:"full"} by default for the complete loaded B-chain semantic DOM baseline. It is the fast page-local observation path and never invokes full CDP DOMSnapshot or a full-page AX tree. params={mode:"text"} returns ALL text from that same full DOM, including offscreen content, as a deduplicated reading view; it is not viewport-only. params={mode:"changes"} is only the inter-action DOM/request journal. Text pages are fixed at 20,000 characters and full pages at 40,000 characters. changes has no interactive UIDs and does not replace the baseline. Query its request IDs with getHttpRequests. nextCursor pages one frozen result: continue with the same mode and exact cursor and never scroll for pagination.' : '',
    snapshotMode ? '- Use searchSnapshot to search the complete frozen B-chain snapshot. It does not scroll, consume DOM mutations, invoke full DOMSnapshot, or invalidate nextCursor. Use includeAx=true only for one exact UID or a narrow ambiguous result. Use includeShadow=true only with one exact shadow-host UID; it performs a bounded local CDP pierce plus local AX and returns the same dom-* UID namespace. A frozen cursor survives waiting and arbitrary asynchronous DOM changes; only a UI-affecting browser interaction or an explicit fresh takeSnapshot invalidates it. Use only UIDs from the latest snapshot.' : '',
