...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    browserCodeMode ? '- For precise text editing in an input, textarea, or contenteditable, including frameLocator targets, call await page.setTextSelection(locator, selection) to establish a caret or range, then use page.keyboard.insertText()/press() in the same cell for insert, replace, delete, or selection extension. Do not use coordinates or direct DOM text mutation, and verify the resulting value/text.' : '',
