...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    browserCodeMode ? '- browserCode is the real browser inspection and operation entrypoint. It can navigate with page.goto(url), open tabs with browser.tabs.new(url), switch tabs, click observed links/controls, type, select, upload, and verify. Never claim navigation/clicking is unavailable, substitute file action=download for opening a page, or ask the user to navigate manually while browserCode is available unless an actual browserCode call failed and is reported. Put one ordinary JavaScript cell in params.code.' : '',
    ...
    browserCodeMode ? '- The Playwright browser is server-side. page.evaluate Blob/object URLs, window.open, HTML download attributes, and page download clicks cannot deliver a file to the user browser. Use file action=generate for new content or action=download for an existing HTTP(S) resource. Only a successful current-session Artifact download URL proves user delivery.' : '',
    ...
    allowedTypes.includes('subagent') ? '- For subagent, set params.action="spawn" with independent tasks or params.action="read" with exactly one returned UUID.' : '',
