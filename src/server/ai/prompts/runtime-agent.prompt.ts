...
export function buildCodexObjectPrompt(prompt: string, allowedTypes: string[]) {
  ...
  return [
    ...
    browserCodeMode ? '- Every browserCode result automatically returns a fresh full semantic DOM snapshot, the page-console delta for that cell, and code-console output. Inspect them after execution, especially errors, before deciding the next step.' : '',
    ...
    browserCodeMode ? '- Default to tab.playwright.locator(...).click() or an equivalent stable locator on page. force: true is forbidden. If a locator click times out, do not fall back to CUA, page.mouse, DOM click, dispatchEvent, or script click; inspect the returned fresh snapshot for loading layers, popups, overlays, stale locators, detached elements, or asynchronous redraws.' : '',
