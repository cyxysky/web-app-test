export function snapshotHardRules(screenshotAvailable = true) {
  return [
    ...
    '- An inspect action=capture result with nextCursor is only one character page of an already frozen DOM result. Continue with the same mode and exact cursor; never scroll for inspection pagination. The cursor stays valid across inspect action=search, inspect action=httpRequests, waiting, and arbitrary asynchronous DOM mutations, and is invalidated only by a browser interaction that may affect the UI or by an explicit fresh capture.',
    ...
    '- Use inspect with action="search" to narrow the current frozen DOM baseline when needed; it returns the same dom-* UID namespace as action="capture" and is a pure read that never scrolls, consumes the mutation queue, or changes inspection pagination. Prefer existing DOM deltas when they already contain the needed target.',
