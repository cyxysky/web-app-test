export function snapshotHardRules(screenshotAvailable = true) {
  return [
    ...
    '- Use searchSnapshot to narrow the current frozen B-chain DOM baseline when needed; it returns the same dom-* UID namespace as takeSnapshot and is a pure read that never scrolls, consumes the mutation queue, or changes snapshot pagination. It also never invokes full DOMSnapshot. Prefer existing DOM deltas when they already contain the needed target.',
    '- If lightweight DOM semantics are genuinely ambiguous, call searchSnapshot with one exact uid or a narrow query and includeAx=true. This performs only bounded local AX reads. Never request AX enrichment for a broad page search.',
    '- If one exact DOM UID is a shadow host whose internals are missing, call searchSnapshot with that uid and includeShadow=true. This performs only a bounded local CDP pierce and local AX enrichment; returned descendants remain in the same dom-* UID namespace.',
    ...
    screenshotAvailable
      ? '- mouse and keyboard accept either one fresh B-chain UID or coordinates from the latest viewport screenshot, never both. takeScreenshot({markers:true}) labels visible elements with those same dom-* UIDs.'
