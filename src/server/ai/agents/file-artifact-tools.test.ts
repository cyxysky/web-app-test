...
test('persists one complete UNO draft and produces the rendered artifact', async (context) => {
  ...
  try {
    ...
    const payload = JSON.parse(result.actual || '{}') as { artifactId?: string; path?: string; qualityGate?: { structural?: boolean } };
    assert.equal(payload.artifactId, 'chat_test/generated/report.docx');
    ...
    const replacement = await editUnoFileArtifact({
      ...
      edits: [{
        oldText: "    document.Text.insertString(cursor, 'Generated through the UNO worker', False)",
...
test('applies unambiguous editor-style text edits in order', () => {
  const source = 'def create_document(job):\n    return "old"\n';
  assert.equal(applyUnoDraftTextEdits(source, [
    { oldText: 'return "old"', newText: 'return "new"' },
    { oldText: 'def create_document(job):', newText: 'def create_document(job):\n    # edited' },
  ]), 'def create_document(job):\n    # edited\n    return "new"\n');
  assert.throws(() => applyUnoDraftTextEdits('same\nsame\n', [
    { oldText: 'same', newText: 'changed' },
  ]), /matched 2 locations/);
  assert.equal(applyUnoDraftTextEdits('same\nsame\n', [
    { oldText: 'same', newText: 'changed', replaceAll: true },
  ]), 'changed\nchanged\n');
});

test('requires editor-style changes after the initial draft without caller revisions', async () => {
  ...
  try {
    ...
    const edited = await editUnoFileArtifact({
      documentId: 'editor-workflow',
      edits: [{ oldText: 'Generated through the UNO worker', newText: 'targeted edit' }],
      render: false,
      runId: 'chat_test',
    });
