...
function compactOlderDomUpdates(messages: ModelMessage[]): ModelMessage[] {
  ...
  for (const [messageIndex, message] of messages.entries()) {
    ...
    for (const [partIndex, rawPart] of message.content.entries()) {
      ...
      if (part.type === 'tool-result' && hasFullDomUpdate) locations.push({ messageIndex, partIndex });
    }
  }
  if (locations.length < 2) return messages;
  const latest = locations.at(-1)!;
  const removeLocations = new Map<number, Set<number>>();
  for (const location of locations) {
    if (location.messageIndex === latest.messageIndex && location.partIndex === latest.partIndex) continue;
    const indexes = removeLocations.get(location.messageIndex) || new Set<number>();
    indexes.add(location.partIndex);
    removeLocations.set(location.messageIndex, indexes);
  }
  return messages.map((message, messageIndex) => {
    const indexes = removeLocations.get(messageIndex);
    ...
    const content = message.content.map((rawPart, partIndex) => {
      ...
      const durableValue = { ...value };
      delete durableValue.domChanges;
      return {
        ...part,
        output: { ...output, value: durableValue },
      } as unknown as typeof rawPart;
    });
...
/** Keep only the newest full DOM update in DOM mode. */
