...
function normalizedSessionId(value: unknown) {
  ...
  if (
    ...
    throw new Error(
      "agent.state is unavailable because the browser conversation state scope is invalid.",
    );
  }
  return sessionId;
}
...
function normalizedKey(value: unknown, name = "agent.state key") {
  ...
  if (
    !key ||
    key.length > BROWSER_CODE_RUNTIME_STATE_MAX_KEY_CHARS ||
    /[\u0000-\u001f\u007f]/.test(key)
  ) {
    throw new Error(
      `${name} must be 1-${BROWSER_CODE_RUNTIME_STATE_MAX_KEY_CHARS} printable characters.`,
    );
  }
  return key;
}
...
function normalizedPrefix(value: unknown) {
  ...
  if (
    typeof value !== "string" ||
    value.length > BROWSER_CODE_RUNTIME_STATE_MAX_KEY_CHARS ||
    /[\u0000-\u001f\u007f]/.test(value)
  ) {
    throw new Error(
      `agent.state prefix must contain at most ${BROWSER_CODE_RUNTIME_STATE_MAX_KEY_CHARS} printable characters.`,
    );
  }
  return value;
}
...
function optionalRevision(value: unknown) {
  ...
  if (
    ...
    throw new Error(
      "agent.state expectedRevision must be a non-negative safe integer.",
    );
  }
  return revision;
}
...
function optionalTtlMs(value: unknown) {
  ...
  if (
    ...
    ttlMs < 1_000 ||
    ttlMs > BROWSER_CODE_RUNTIME_STATE_MAX_TTL_MS
  ) {
    throw new Error(
      `agent.state ttlMs must be an integer between 1000 and ${BROWSER_CODE_RUNTIME_STATE_MAX_TTL_MS}.`,
    );
  }
  return ttlMs;
}
...
function serializeRuntimeStateValue(value: unknown) {
  ...
  const visit = (item: unknown, depth: number): void => {
    if (depth > 30)
      throw new Error(
        "agent.state value exceeds the maximum nesting depth of 30.",
      );
    if (item === null || typeof item === "string" || typeof item === "boolean")
      return;
    if (typeof item === "number") {
      if (!Number.isFinite(item))
        throw new Error("agent.state value numbers must be finite.");
      return;
    }
    ...
    if (seen.has(item))
      throw new Error(
        "agent.state value must not contain circular references.",
      );
    seen.add(item);
    if (Array.isArray(item)) {
      ...
      if (prototype !== Object.prototype && prototype !== null) {
        throw new Error(
          "agent.state value objects must be plain JSON records.",
        );
      }
  ...
  if (serialized === undefined)
    throw new Error("agent.state value must be JSON-serializable.");
  if (
    Buffer.byteLength(serialized, "utf8") >
    BROWSER_CODE_RUNTIME_STATE_MAX_VALUE_BYTES
  ) {
    throw new Error(
  ...
  const oversizedBase64 =
    /"(?:b64|base64|data)"\s*:\s*"(?:data:[^;"]+;base64,)?[A-Za-z0-9+/=]{65536,}"/i.test(
      serialized,
    ) || /"data:[^;"]+;base64,[A-Za-z0-9+/=]{65536,}"/i.test(serialized);
  if (oversizedBase64) {
    throw new Error(
      "agent.state must not store large Base64 payloads. Save the bytes as a workspace artifact file and store only its asset name.",
    );
  }
  return serialized;
}

async function purgeExpiredState(database: DatabaseExecutor, timestamp: string) {
...
function stateEntry(row: RuntimeStateRow) {
  return {
    key: row.key,
    value: parseDatabaseJson<unknown>(row.value_json, null),
...
function currentRow(database: DatabaseExecutor, sessionId: string, key: string) {
  return queryDatabaseOne<RuntimeStateRow>(`
...
): Promise<{ items: BrowserCodeRuntimeStateEntry[]; count: number; truncated: boolean }>;
export function executeBrowserCodeRuntimeStateOperation(
  rawSessionId: string,
  operation: BrowserCodeRuntimeStateOperation & { action: "clear" },
): Promise<{ deleted: number; prefix: string }>;
export function executeBrowserCodeRuntimeStateOperation(
  rawSessionId: string,
...
export async function executeBrowserCodeRuntimeStateOperation(
  rawSessionId: string,
  operation: BrowserCodeRuntimeStateOperation,
): Promise<
  | BrowserCodeRuntimeStateEntry
  ...
  const input = inputRecord(
    operation.input ?? {},
    `agent.state.${operation.action} input`,
  );
  ...
  if (operation.action === "get") {
    ...
    return runDatabaseTransaction(async (database) => {
      await purgeExpiredState(database, timestamp);
      const row = await currentRow(database, sessionId, key);
      return row ? { found: true, ...stateEntry(row) } : { found: false, key };
    });
  }

  if (operation.action === "set") {
    rejectUnknownFields(input, "agent.state.set input", [
      "key",
      "value",
      "expectedRevision",
      "ttlMs",
    ...
    const result = await runDatabaseTransaction(async (database) => {
      await purgeExpiredState(database, timestamp);
      const existing = await currentRow(database, sessionId, key);
      ...
      if (
        ...
        throw new Error(
          `agent.state revision conflict for key ${JSON.stringify(key)}: expected ${expectedRevision}, current ${currentRevision}.`,
        );
      }
      const aggregate = await queryDatabaseOne<{ key_count?: number }>(`
        SELECT COUNT(*) AS key_count
        FROM browser_code_runtime_state
        WHERE session_id = ? AND namespace = ?
      `, [sessionId, runtimeStateNamespace], database);
      const keyCount = Number(aggregate?.key_count) || 0;
      if (!existing && keyCount >= BROWSER_CODE_RUNTIME_STATE_MAX_KEYS) {
        throw new Error(
          `agent.state stores at most ${BROWSER_CODE_RUNTIME_STATE_MAX_KEYS} keys per conversation.`,
        );
      }
      ...
      return {
        key,
        value: parseDatabaseJson<unknown>(valueJson, null),
        revision,
        updatedAt: timestamp,
        ...(expiresAt ? { expiresAt } : {}),
      };
  ...
  if (operation.action === "delete") {
    rejectUnknownFields(input, "agent.state.delete input", [
      "key",
      "expectedRevision",
    ]);
    const key = normalizedKey(input.key);
  ...
  if (operation.action === "list") {
    ...
    const requestedLimit =
      input.limit === undefined
        ? BROWSER_CODE_RUNTIME_STATE_MAX_KEYS
        : Number(input.limit);
    if (
      !Number.isInteger(requestedLimit) ||
      requestedLimit < 1 ||
      requestedLimit > BROWSER_CODE_RUNTIME_STATE_MAX_KEYS
    ) {
      throw new Error(
        `agent.state list limit must be an integer between 1 and ${BROWSER_CODE_RUNTIME_STATE_MAX_KEYS}.`,
      );
    }
    const result = await runDatabaseTransaction(async (database) => {
      await purgeExpiredState(database, timestamp);
      const matching = (await queryDatabase<RuntimeStateRow>(`
        SELECT key, value_json, revision, updated_at, expires_at
        FROM browser_code_runtime_state
        WHERE session_id = ? AND namespace = ?
        ORDER BY key ASC
      `, [sessionId, runtimeStateNamespace], database)).filter((row) => row.key.startsWith(prefix));
      return {
  ...
  if (operation.action === "clear") {
    ...
    const result = await runDatabaseTransaction(async (database) => {
      await purgeExpiredState(database, timestamp);
      const keys = (await queryDatabase<{ key: string }>(`
      ...
      `, [sessionId, runtimeStateNamespace, ...keys], database);
  ...
  throw new Error(
    `Unsupported agent.state action: ${String(operation.action)}.`,
  );
}
